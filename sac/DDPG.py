...
class QFunction(Feedforward):
    def __init__(self, observation_dim, action_dim, hidden_sizes=[100,100],
        ...
        super().__init__(observation_dim+action_dim, hidden_sizes, 1)
...
class DDPGAgent(object):
    ...
    def __init__(self, observation_space, action_space, **userconfig):
        ...
        scale = high - low
        offset = high + low
        output_activation = lambda x: 0.5 * (scale * tanh(x) + offset)
    ...
    def act(self, observation, eps=None):
        ...
        obs_tens = torch.from_numpy(observation.astype(np.float32))
        action = self.policy.forward(obs_tens).detach().numpy()
    ...
    def train(self, iter_fit=32):
        ...
        for i in range(iter_fit):
            ...
            q_target = rew + self._discount * (1-done) * self.Q_target.Q_value(s_prime, a_prime)
            q_loss_value = self.Q.fit(s, a, q_target)


            self.policy.train()
            self.optimizer.zero_grad()
            
            a_hat = self.policy.forward(s)
            actor_loss = -torch.mean(self.Q.Q_value(s, a_hat))
...
def main():
    ...
    def save_statistics():
        with open(f"./results/DDPG_{env_name}-eps{eps}-u{up_freq}-l{lr}-s{random_seed}-stat.pkl", 'wb') as f:
            pickle.dump({"rewards" : rewards, "lengths": lengths, "eps": eps, "train": train_iter,
                         "lr": lr, "update_every": up_freq, "losses": losses}, f)
    ...
    for i_episode in range(1, max_episodes+1):
        ...
        if i_episode % 500 == 0:
            ...
            torch.save(ddpg.state(), f'./results/DDPG_{env_name}_{i_episode}-eps{eps}-u{up_freq}-l{lr}-s{random_seed}.pth')
            save_statistics()
